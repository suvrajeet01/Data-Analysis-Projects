...
def do_algorithm():
    ...
    #evaluate(algo_1, data, measures=['RMSE', 'MAE']) #检验'RMSE'
    #evaluate(algo_2, data, measures=['RMSE', 'MAE']) #检验'RMSE'
...
#基于资助关系算法所用到的函数#
    ...
    #使用svd算法
    trainset_2 = data.build_full_trainset()
    algo_2.train(trainset)
    testset_2 = trainset.build_anti_testset()
    ...
    if use_knn == True:
        ...
        predictions_1 = new_prediction(predictions_1)
    top_n = get_top_n(predictions, n=5)
    
    top_n_1 = get_top_n(predictions_1, n=5)
    top_n_2 = get_top_n(predictions_2, n=5)
    ...
    for uid, user_ratings in top_n_2.items():
        final_dict_2[get_user_name(uid)] = [get_item_name(iid) for (iid, _) in user_ratings] #得到最后的对应结果

    with open('data/knn_basic.pickle', 'wb') as handle:
        pickle.dump(final_dict, handle)

    with open('data/knn_2.pickle', 'wb') as handle:
        pickle.dump(final_dict_1, handle)
        
    with open('data/svd.pickle', 'wb') as handle:
        pickle.dump(final_dict_2, handle)

    return final_dict, final_dict_1, final_dict_2
...
def use_already_data():
    ...
    with open("data/knn_2.pickle", "rb") as myFile:
        final_dict1 = pickle.load(myFile)
    with open("data/svd.pickle", "rb") as myFile:
        final_dict2 = pickle.load(myFile)
    return final_dict,final_dict1,final_dict2
...
def find_nearest(input_list):
    ...
    #print("最相似的基金会：" + data['基金会名字'][index_min])
...
while(num_tag == True):
    if num == "1":
        final_dict,final_dict1, final_dict2 = do_algorithm()
        num_tag = False
    elif num =="2":
        final_dict,final_dict1,final_dict2 = use_already_data()
        num_tag = False
    else:
        num = input("请输入正确数字！")
