...
current_num = 0
while(go== True):
    search_url = "http://search.jiayuan.com/v2/index.php?key=&sex=f&stc=1:3311,23:1&sn=default&sv=1&pt=22&ft=off&f=select&mt=d&p=" + str(page)
    driver.get(search_url)
    time.sleep(2)
    html = driver.execute_script("return document.getElementsByTagName('html')[0].innerHTML")
    soup=BeautifulSoup(html,'lxml')
    final = geturl(soup,df).drop_duplicates().reindex()
    final.to_csv('lishui.csv', encoding='utf-8')
